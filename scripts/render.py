            ...
            for rendered_output_name in rendered_output_names:
                ...
                if rendered_output_name == "depth":
                    if near_plane is None or far_plane is None:
                        near_plane = np.percentile(np.array(output_image).flatten(), 10)
                        far_plane = np.percentile(np.array(output_image).flatten(), 90)
                        CONSOLE.log(f"Adaptive setting near plane to {near_plane} and far plane to {far_plane} by percentile 10/90 of the first image")
                    output_image = (output_image - near_plane) / (far_plane - near_plane)
                    output_image = np.clip(output_image, 0, 1)
                    output_image = apply_colormap(torch.tensor(output_image)).numpy()
                render_image.append(output_image)
            
            if output_format == "images":
                for i, name in enumerate(rendered_output_names):
                    if name == 'rgb':
                        media.write_image(output_image_dir / f"rgb_{camera_idx:05d}.png", render_image[i])
                    elif name == 'depth':
                        media.write_image(output_image_dir / f"depth_{camera_idx:05d}.png", render_image[i])
                    else:
...
class RenderTrajectory:
    ...
    def main(self) -> None:
        ...
        if self.traj == "filename":
            ...
            camera_path = get_path_from_json(camera_path, pipeline.datamanager.eval_dataset.cameras.probe_config)
        else:
