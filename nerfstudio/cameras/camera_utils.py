...
def get_interpolated_poses_many(
    ...
    return torch.tensor(traj), torch.tensor(Ks_new)
...
def auto_orient_and_center_poses(
    ...
    if method == "pca":
        ...
        transform = torch.cat([eigvec.T, eigvec.T @ -translation[..., None]], dim=-1)
        oriented_poses = transform @ poses

        if oriented_poses.mean(axis=0)[2, 1] < 0:
            oriented_poses[:, 1:3] = -1 * oriented_poses[:, 1:3]
            revert_matrix = torch.diag(torch.Tensor([1, -1, -1]))
            transform = revert_matrix @ transform
    elif method == "up":
