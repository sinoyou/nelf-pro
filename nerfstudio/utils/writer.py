# zinyou note
# This file entirely acts as a 'writer class'. All messages are stored in buffers in advanced and then flushed by specific writers. 
# Writer file has buffers EVENT_STORAGE and GLOBAL_BUFFER.
# EVENT_STORAGE stores messages to be printed locally / remotely.
# GLOBAL_BUFFER stores some configurations of local writer and time information (for extraction from viewer)
# EVENT_STORAGE is written by put_scalar(), put_dict(), put_image(), etc.
# EVENT_STORAGE is read and displayed by write_out_storage().
# remote writers are configured in setup_event_writer().
# local writers are configured in setup_local_writer().
...
def put_plotly(name: str, figure: Any, step: int = 0):
    """Setter function to place plotly figure into the queue to be written out
    
    Args:
        figure: plotly figure to write out
    """
    EVENT_STORAGE.append({"name": name, "write_type": EventType.PLOTLY, "event": figure, "step": step})
...
class WandbWriter(Writer):
    ...
    def __init__(self, log_dir: Path, experiment_name: str):
        wandb.init(project="NeLF-Pro", name=experiment_name, dir=str(log_dir), reinit=True)
    ...
    def write_plotly(self, name: str, figure: Any, step: int) -> None:
        wandb.log({name: wandb.Plotly(figure)}, step=step)
...
class TensorboardWriter(Writer):
    ...
    def write_plotly(self, name: str, figure: Any, step: int) -> None:
        self.tb_writer.add_scalar(name + "is not implemented yet", 0.0, 0)
