...
@dataclass
class MultiStepSchedulerWithWarmupConfig(InstantiateConfig):
    """Basic scheduler config with self-defined linear warmup schedule and exponential decay schedule"""

    _target: Type = field(default_factory=lambda: lr_scheduler.ChainedScheduler)
    ...
    def setup(self, optimizer=None, lr_init=None, **kwargs) -> Any:
        ...
        warmup_scheduler = lr_scheduler.LinearLR(
            optimizer, start_factor=0.01, total_iters = self.warm_up_steps, **kwargs
        )
        main_scheduler = lr_scheduler.MultiStepLR(
            optimizer, 
            milestones=[self.max_steps // 2, self.max_steps * 3 // 4, self.max_steps * 9 // 10],
            gamma=0.33,
            **kwargs
        )
        return self._target([warmup_scheduler, main_scheduler])